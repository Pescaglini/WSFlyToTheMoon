...
export class SceneManager<R extends IRenderer> {
	...
	public readonly initializedAssets: Promise<void>;
	...
	private async initAssets(): Promise<void> {
		...
		// remove all .meta files - Hack until assetpack allows me to not manifest them to begin with
		for (const bundle of manifest.bundles) {
			if (Array.isArray(bundle.assets)) {
				bundle.assets = bundle.assets.filter((asset: any) => !asset.srcs[0].endsWith(".meta"));
			} else {
				for (const assetName in bundle.assets) {
					if (assetName.endsWith(".meta")) {
						bundle.assets[assetName] = undefined;
					}
				}
			}
		}
