import type { AssetExtension, ResolvedAsset, Loader } from "@pixi/assets";
...
const langAssetExtension: AssetExtension = {
	...
	loader: {
		...
		testParse(_asset: any, options: ResolvedAsset): Promise<boolean> {
			return Promise.resolve(options.src.includes("i18.json") && options.data.lng && options.data.ns);
		},

		parse(asset: any, options: ResolvedAsset, _loader: Loader): Promise<any> {
