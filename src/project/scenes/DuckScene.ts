...
export class DuckScene extends PixiScene {
	...
	constructor() {
		...
		const duckModel = Assets.get("models/duck/Duck.gltf");
		...
		const regularSprite = Sprite.from("img/big_placeholder/bg_game.png");
		this.addChild(regularSprite);

		const dancingStormtrooperObject = Model.from(Assets.get("models/dancing-stormtrooper.glb"));
